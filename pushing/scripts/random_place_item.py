        ...
        if current_time - msg_time < 2.0:
            try:
                ...
                # Transform camera coordinates to world coordinates using tf
                    ...
                    self.gripper.grasp()
                    # x,y,rot = new_obj_pos
                    x = 20+(np.random.randint(12))
                    y = 20+(np.random.randint(12))
                    rot = np.random.randint(12)*tau/12
                    print('new item position:',x,y,rot)
                    num_file = 1
                    new_item_pose = [x,y,rot]
                    file_list = os.listdir("./data/validation_results/real_robot/placing_comparison_method/")
                    file_name = 'initial_item_pose_'+str(num_file)+".pkl"
                    while file_name in file_list:
                        num_file +=1
                        file_name = 'initial_item_pose_'+str(num_file)+".pkl"
                    file_path = "./data/validation_results/real_robot/placing_comparison_method/"+file_name
                    f_save = open(file_path,'wb')
                    pkl.dump(new_item_pose,f_save)
                    f_save.close()
                    if rot <-tau/2:
                        rot = rot + tau
                    elif rot >tau/2:
